...
<body>
	...
	<div class="page-container">
		...
		function operate_sale_report_list(){   
			$(document).ready(function(){   
				...
				$.ajax({
					method: "POST", 
					url: "<?php echo site_url('/sale_report/fetch_sale_status_data/'); ?>",
		...
		function operate_leads_status_echart(paras1){
			$(document).ready(function(){   
				...
				var is_deleted = status_arrs[1];
				var is_rejected = status_arrs[2];
				var is_no_show = status_arrs[3];
				var total_bookings = status_arrs[4];
				...
				var perc_is_deleted = 0;
				var remaining_is_deleted = 0; 
				if(total_bookings>0){  /* is_deleted>0 && total_bookings>0 */ 
					perc_is_deleted = (is_deleted/total_bookings)*100;
					perc_is_deleted = perc_is_deleted.toFixed(0);
					remaining_is_deleted = 100 - perc_is_deleted; 
				}
				
				var perc_is_rejected = 0;
				var remaining_is_rejected = 0;  
				if(total_bookings>0){  /* is_rejected>0 && total_bookings>0 */
					perc_is_rejected = (is_rejected/total_bookings)*100;
					perc_is_rejected = perc_is_rejected.toFixed(0); 
					remaining_is_rejected = 100 - perc_is_rejected; 
				}
				
				var perc_is_no_show = 0;
				var remaining_is_no_show = 0;  
				if(total_bookings>0){ /* is_no_show>0 && total_bookings>0 */
					perc_is_no_show = (is_no_show/total_bookings)*100;
					perc_is_no_show = perc_is_no_show.toFixed(0);
					remaining_is_no_show = 100 - perc_is_no_show;
				}
					...
					function (ec, limitless) { 
						...
						multiple_donuts_options = {
							...
							legend: {
								x: 'center',
								y: '-20%',
								data: ['Confirm','Delete','Reject','No Show']
							},
							...
							series: [ 
								{
									...
									data: [
										{name: 'other', value : remaining_is_deleted, itemStyle: labelBottom},
										{name: 'Confirm ('+is_deleted+')', value : perc_is_deleted, itemStyle: labelTop}
									]
								},  
								{
									type: 'pie',
									...
									data: [
										...
										{name:'Delete ('+in_process+')', value : perc_in_process, itemStyle: labelTop}
									]
								}, 
								{ 
									type: 'pie',
									...
									itemStyle: labelFromatter,
									data: [
										{name: 'other', value : remaining_is_rejected, itemStyle: labelBottom},
										{name: 'Reject ('+is_rejected+')', value : perc_is_rejected,itemStyle: labelTop}
									]
								},  
								{
									type: 'pie',
									...
									itemStyle: labelFromatter,
									data: [
										{name:'other', value : remaining_is_no_show, itemStyle: labelBottom},
										{name:'No Show ('+is_no_show+')', value : perc_is_no_show,itemStyle: labelTop}
									]
								} 
							]
						};
       ...
       <form name="datas_form1" id="datas_form1" method="post" action="" class="form-horizontal"> 
            ...
            <!--<div class="form-group">  -->   
                <div class="col-lg-3">  
                   <select name="years" id="years" class="form-control input-sm mb-md cstm_select2" onChange="operate_sale_report_list();">
                ...
                <div class="col-lg-3">   
                  <select name="months" id="months" class="form-control input-sm mb-md cstm_select2" onChange="operate_sale_report_list();">
	 ...
	 <?php   //Confirm  Delete  Reject  No Show
        $in_confirm = $this->sale_report_model->get_nos_of_bookings_by_status(array(),'1'); 
         
        $is_deleted = $this->sale_report_model->get_nos_of_bookings_by_status(array(),'2'); 
        
        $is_rejected = $this->sale_report_model->get_nos_of_bookings_by_status(array(),'3'); 
        
        $is_no_show = $this->sale_report_model->get_nos_of_bookings_by_status(array(),'4'); 
        
        $total_bookings = $this->sale_report_model->get_nos_of_bookings_by_status(array(),''); 
		...
		$perc_is_deleted = $remaining_is_deleted = '0';
		if($total_bookings>0){ /* $is_deleted>0 && $total_bookings>0 */
			$perc_is_deleted = ($is_deleted/$total_bookings)*100;
			$perc_is_deleted = number_format($perc_is_deleted,0);
			$remaining_is_deleted = 100 - $perc_is_deleted; 
        }
		
		$perc_is_rejected = $remaining_is_rejected = '0';
		if($total_bookings>0){ /* $is_rejected>0 && $total_bookings>0 */
			$perc_is_rejected = ($is_rejected/$total_bookings)*100;
			$perc_is_rejected = number_format($perc_is_rejected,0); 
			$remaining_is_rejected = 100 - $perc_is_rejected; 
        }
		
		$perc_is_no_show = $remaining_is_no_show = '0';
		if($total_bookings>0){ /* $is_no_show>0 && $total_bookings>0 */
			$perc_is_no_show = ($is_no_show/$total_bookings)*100;
			$perc_is_no_show = number_format($perc_is_no_show,0);
			$remaining_is_no_show = 100 - $perc_is_no_show; 
		} ?> 
                ...
                function (ec, limitless) { 
                    ...
                    multiple_donuts_options = {
                        ...
                        legend: {
                            x: 'center',
                            y: '-20%',
                            data: ['Confirm','Delete','Rejected','No Show']
                        },
                        ...
                        series: [{
                                ...
                                itemStyle: labelFromatter,
                                data: [
									{name: 'other', value: <?php echo $remaining_is_deleted; ?>, itemStyle: labelBottom},
                                    {name: 'Delete (<?php echo $is_deleted; ?>)', value: <?php echo $perc_is_deleted; ?>, itemStyle: labelTop}
								]
                            }, { 
                                type: 'pie',
                                center: ['23%', '82.5%'],
                                ...
                                itemStyle: labelFromatter,
                                data: [
                                    {name: 'other', value: <?php echo $remaining_is_rejected; ?>, itemStyle: labelBottom},
                                    {name: 'Rejected (<?php echo $is_rejected; ?>)', value: <?php echo $perc_is_rejected; ?>,itemStyle: labelTop}
                                ]
                            }, {
                                type: 'pie',
                                center: ['75%', '82.5%'],
                                ...
                                itemStyle: labelFromatter,
                                data: [
                                    {name:'other', value:<?php echo $remaining_is_no_show; ?>, itemStyle: labelBottom},
                                    {name:'No Show (<?php echo $is_no_show; ?>)', value:<?php echo $perc_is_no_show; ?>,itemStyle: labelTop}
                                ]
                            } 
                        ]
                    };
