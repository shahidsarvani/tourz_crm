<?php
	...
	function index(){  
		...
		if($res_nums>0){
			...
			//total rows count
			$totalRec = count($this->commission_report_model->get_all_filter_packages($paras_arrs));
			
			//pagination configuration
			$config['target']      = '#fetch_dya_list';
			$config['base_url']    = site_url('/commission_report/index2');
			...
			$records = $data['records'] = $this->commission_report_model->get_all_filter_packages($paras_arrs);
	...
	function index2(){
		...
		if($res_nums>0){
		   ...
		   $data['records'] = $this->commission_report_model->get_all_filter_packages($paras_arrs); 
			 
			$this->load->view('commission_report/index2',$data); 
		}else{ 
			$this->load->view('no_permission_access'); 
		}
	} 
	...
	function trash_aj(){ 
		...
		$res_nums =  $this->general_model->check_controller_method_permission_access('Commission_report','trash',$this->login_vs_role_id,'1');
		if($res_nums>0){  
		
			 if(isset($_POST["args1"]) && $_POST["args1"]>0){
				$args1 = $this->input->post("args1"); 
				$this->commission_report_model->trash_package($args1);  
			 }  
	...
	function add(){   
	 	$res_nums =  $this->general_model->check_controller_method_permission_access('Commission_report','add',$this->login_vs_role_id,'1');  
		if($res_nums>0){
			...
			if(isset($_POST) && !empty($_POST)){
				...
				// validation fail
					$this->load->view('commission_report/add',$data); 
				}else{   
					...
					$res = $this->commission_report_model->insert_package_data($datas); 
					if(isset($res)){  
					...
					if(isset($_POST['saves_and_new'])){
						redirect("commission_report/add");
					}else{
						redirect("commission_report/index");	
					} 
				} 	 
	 ...
	 function update($args1=''){  
	 	$res_nums =  $this->general_model->check_controller_method_permission_access('Commission_report','update',$this->login_vs_role_id,'1');
		if($res_nums>0){ 
		
			$data['page_headings'] = 'Update Package';
			if(isset($args1) && $args1!=''){ 
				$data['args1'] = $args1; 
				$data['record'] = $this->commission_report_model->get_package_by_id($args1);
				...
				if(isset($_POST) && !empty($_POST)){
					...
					if($this->form_validation->run() == FALSE){ 
					// validation fail
						$this->load->view('commission_report/update',$data);
						...
						$res = $this->commission_report_model->update_package_data($args1,$datas); 
						if(isset($res)){ 
