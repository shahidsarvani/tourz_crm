<?php
	...
	/* Commission_report module starts */
	function index(){  
		...
		if($res_nums>0){
			...
			//total rows count
			$totalRec = count($this->commission_report_model->get_all_filter_commission($paras_arrs));
			...
			$records = $data['records'] = $this->commission_report_model->get_all_filter_commission($paras_arrs);
	...
	function index2(){
		...
		if($res_nums>0){
		   ...
		   $data['records'] = $this->commission_report_model->get_all_filter_commission($paras_arrs); 
	...
	function commission_detail($args1=''){  
		if($args1>0){ 
		
			$data['page_headings'] = "Commission Detail";	 
			$data['args1'] = $args1; 
			$temp_recs = $data['record'] = $this->commission_report_model->get_commission_by_id($args1);
			...
			if(isset($temp_recs) && $temp_recs->is_new==1){
				...
				//$this->commission_report_model->update_commission_data($args1,$datass);  	 	
			}
			
			$this->load->view('commission_report/commission_detail',$data); 
